// 로스트아크 시세 데이터 (loa-calc가 쓰는 공개 엔드포인트).
// market-price.service.ts(Angular @Injectable)를 순수 함수로 재작성한 것.
const MARKET_PRICE_URL = 'https://market-cron.icepeng.workers.dev';
...
// 모든 재료 키를 0(+ 골드 1)으로 채운 초기 가격맵. 시세 로드 실패 시에도
// optimize/fixed/getReport가 NaN 없이 동작하도록 보장한다.
export function emptyPriceMap(): Record<string, number> {
  return { ...getPriceObj({ updateTime: '', items: [] }, 'RecentPrice'), 골드: 1 };
}

export async function fetchMarketPrices(): Promise<MarketData> {
  const res = await fetch(MARKET_PRICE_URL);
  if (!res.ok) {
    throw new Error(`시세 응답 오류: ${res.status}`);
  }
  return (await res.json()) as MarketData;
}

// 시세 데이터에서 재련/상급 재련 계산기가 쓰는 재료명별 단가 맵을 만든다.
// 일부 재료는 묶음 단위(파편 주머니, 결정 100개 등)라 단위당 가격으로 환산한다.
export function getPriceObj(
  data: MarketData,
  priceType: PriceType
