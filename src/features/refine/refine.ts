import type { RefineTable } from './refineData';
...
// optimize/fixed 경로의 기대 재료 소모량 (재시도 반영).
// 각 시도 도달확률 = globalProb / totalProb. 이 가중합의 골드는 price와 일치.
export function expectedMaterials(
  ...
  for (const st of path) {
    const reach = st.totalProb > 0 ? st.globalProb / st.totalProb : 0;
    for (const [m, a] of Object.entries(amount)) out[m] = (out[m] ?? 0) + a * reach;
    for (const [b, c] of Object.entries(st.breathes))
      out[b] = (out[b] ?? 0) + c * reach;
  }
  return out;
}
...
function subtractAmount(
  ...
  return Object.fromEntries(
    Object.entries(bindedMap)
      .map(([key, amount]): [string, number] => [key, amount - (amountMap[key] ?? 0)])
