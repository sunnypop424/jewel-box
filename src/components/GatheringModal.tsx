...
import { RAID_META } from '../constants'; // ✅ 레이드 목록 가져오기
    ...
    try {
      ...
      const response = await fetch(WORKER_URL, {
        ...
        body: JSON.stringify({
          ...
          time: targetTime,
          raid: raidLabel, 
          memo: memo || '숙제 위주로 빠르게 뺍니다!',
        }),
      });

      if (!response.ok) {
        throw new Error('디스코드 알림 전송에 실패했습니다.');
      }

      alert('디스코드로 모집 알림이 전송되었습니다!');
      onClose();
      // 폼 초기화
      setTargetDate('');
      setTargetRaid('DECIDE_LATER');
      setMemo('');
      
    } catch (error) {
      console.error(error);
      alert('전송 중 오류가 발생했습니다. Worker URL과 상태를 확인해주세요.');
    } finally {
  ...
  return (
    <Modal open={isOpen} title="레이드 파티 모집" onClose={onClose} maxWidth="max-w-3xl">
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 pb-2">
        ...
        {/* ✅ CharacterFormList와 동일한 스타일의 강조 박스 */}
        <div className="flex flex-col gap-4 rounded-2xl border border-indigo-100 bg-indigo-50/30 p-5 dark:border-indigo-900/30 dark:bg-indigo-900/10">
          ...
          <div className="grid gap-4 sm:grid-cols-2 mt-2">
            ...
            <div className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm font-medium text-zinc-700 transition-all focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200"
              />
            </div>
          ...
          <div className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <label className="flex items-center gap-1.5 text-xs font-bold text-zinc-500">
              <Swords size={14} /> 진행 레이드
            </label>
            <div className="relative">
              <select
                value={targetRaid}
                onChange={(e) => setTargetRaid(e.target.value)}
                disabled={isSubmitting}
                className="w-full appearance-none cursor-pointer rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 pr-10 text-sm font-medium text-zinc-700 transition-all focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200"
              >
                <option value="DECIDE_LATER">모여서 정합니다.</option>
                {Object.entries(RAID_META).map(([id, meta]) => (
          ...
          <div className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            <label className="flex items-center gap-1.5 text-xs font-bold text-zinc-500">
              <MessageSquare size={14} /> 추가 메모
            </label>
            <textarea
              value={memo}
              onChange={(e) => setMemo(e.target.value)}
              disabled={isSubmitting}
              placeholder="예) 종막 하드 1~2관문 가실 분 모집합니다!"
              rows={2}
        ...
        {/* 하단 버튼 (CharacterFormList와 동일한 버튼 스타일) */}
        <div className="flex items-center justify-end gap-3 border-t border-zinc-100 pt-6 dark:border-zinc-800">
          <button
            type="button"
            onClick={onClose}
            disabled={isSubmitting}
            className="rounded-xl px-6 py-3 text-sm font-bold text-zinc-500 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800"
          >
            취소
          </button>
          <button
            type="submit"
            disabled={isSubmitting}
            className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-8 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition-colors hover:bg-indigo-500 disabled:opacity-70"
          >
